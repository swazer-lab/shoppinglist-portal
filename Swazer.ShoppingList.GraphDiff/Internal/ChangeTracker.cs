...
namespace Swazer.ShoppingList.GraphDiff.Internal
{
    ...
    internal class ChangeTracker : IChangeTracker
    {
        ...
        public void AttachCyclicNavigationProperty(object parent, object child, List<string> mappedNavigationProperties)
        {
            ...
            if (parentNavigationProperties.Count > 1)
            {
                throw new NotSupportedException(
                    $"Found ambiguous parent navigation property of type '{parentType}'. Map one of the parents ({GetConcatenatedPropertyNames(parentNavigationProperties)}) as an associate to disambiguate.");
            }
        ...
        {
            return properties.Aggregate("", (current, parentProperty) => current + $"'{parentProperty.Name}', ").TrimEnd(',', ' ');
        }
        ...
        public object AttachAndReloadAssociatedEntity(object entity)
        {
            ...
            {
                // look into a possible better way of doing this, I don't particularly like it
        ...
        public void AttachRequiredNavigationProperties(object updating, object persisted)
        {
            ...
            foreach (var navigationProperty in _entityManager.GetRequiredNavigationPropertiesForType(updating.GetType()))
            {
                ...
                {
                    // this is performing a db query - look for alternative.
        ...
        private void EnsureConcurrency(object entity1, object entity2)
        {
            ...
            // EF should do this automatically should it not?
            foreach (var concurrencyProp in concurrencyProperties)
            {
                ...
                {
                    throw new DbUpdateConcurrencyException($"{concurrencyProp.Name} failed optimistic concurrency");
                }
            }
        }
