function ReservationNumberSearchCriteriaVM(options) {
    var self = this;
...
function ReservationNumberVM(id, reservationCount, isDefault, isActive) {
    var self = this;

    self.ReservationNumberId = ko.observable(id);
    self.ReservationCount = ko.observable(reservationCount).extend({ required: true });
    self.IsDefault = ko.observable(isDefault).extend({ required: true });
    self.IsActive = ko.observable(isActive).extend({ required: true });

    self.copy = function () {
        return new ReservationNumberVM(self.ReservationNumberId(), self.ReservationCount(), self.IsDefault(), self.IsActive());
    }

    self.toSubmitModel = function () {
        return {
            ReservationNumberId: self.ReservationNumberId(),

            ReservationCount: self.ReservationCount(),
    ...
    self.FullValidation = function () {
        if (self.ReservationCount() === undefined || self.ReservationCount() === '')
            return false;
...
function ReservationNumberMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            return new ReservationNumberVM(item.ReservationNumberId, item.ReservationCount, item.IsDefault, item.IsActive);
        });
    };
    ...
    self.ReservationNumbers = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.ReservationNumbers().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new ReservationNumberVM(0, 0, false, false);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.ReservationNumbers(), function (element) {
            return element.ReservationNumberId() === self.SelectedElement().ReservationNumberId();
        });
    };
    ...
    self.GetSelectedElementOrFirstOrNew = function () {
        ...
        if (self.IsItemsEmpty() === false) {
            self.EnterDetailsMode();
            return self.ReservationNumbers()[0];
        }
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.ReservationNumbers(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.ReservationNumbers(), function (temp) {
            return temp.ReservationNumberId() === id;
        });
    }
    ...
    self.Delete = function () {
        ...
        $.ajax({
            ...
            url: self.DeleteUrl,
            data: {
                id: self.SelectedElement().ReservationNumberId(),
                criteria: self.SearchCriteria().toSubmitModel()
            },
            success: function (e) {
                self.HandleSuccess(e);
