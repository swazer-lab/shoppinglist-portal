function WorkHourSearchCriteriaVM(options) {
    var self = this;
...
function WorkHourVM(id, hour, isActive, workPeriod) {
    var self = this;

    self.WorkHourId = ko.observable(id);
    self.Hour = ko.observable(hour).extend({ required: true, digit: true, min: 0, max: 23 });
    self.IsActive = ko.observable(isActive).extend({ required: true });
    self.WorkPeriod = ko.observable(workPeriod).extend({ required: true });

    self.copy = function () {
        return new WorkHourVM(self.WorkHourId(), self.Hour(), self.IsActive(), self.WorkPeriod());
    }

    self.toSubmitModel = function () {
        return {
            WorkHourId: self.WorkHourId(),
            Hour: self.Hour(),
            IsActive: self.IsActive(),
            WorkPeriodId: self.WorkPeriod().WorkPeriodId,
        };
    };
    ...
    self.FullValidation = function () {
        if (self.Hour() === undefined || self.Hour() === '')
            return false;
...
function WorkHourMainVM(options) {
    var self = this;
    ...
    self.SearchWorkPeriods = function (workPId) {
        return ko.utils.arrayFirst(self.WorkPeriods(), function (wp) {
            return wp.WorkPeriodId === workPId;
        });
    };

    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            var wp = self.SearchWorkPeriods(item.WorkPeriodId);
            return new WorkHourVM(item.WorkHourId, item.Hour, item.IsActive, wp);
        });
    };
    ...
    self.WorkHours = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.WorkHours().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new WorkHourVM(0, 0, false, undefined);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.WorkHours(), function (element) {
            return element.WorkHourId() === self.SelectedElement().WorkHourId();
        });
    };
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.WorkHours(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.WorkHours(), function (temp) {
            return temp.WorkHourId() === id;
        });
    }
    ...
    self.Delete = function () {
        ...
        $.ajax({
            ...
            url: self.DeleteUrl,
            data: {
                id: self.SelectedElement().WorkHourId(),
                criteria: self.SearchCriteria().toSubmitModel()
            },
            success: function (e) {
                self.HandleSuccess(e);
