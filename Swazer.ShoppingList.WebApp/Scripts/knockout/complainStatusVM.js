function ComplainStatusSearchCriteriaVM(options) {
    var self = this;
...
function ComplainStatusVM(id, arabicName, englishName) {
    var self = this;

    self.ComplainStatusId = ko.observable(id);
    ...
    self.copy = function () {
        return new ComplainStatusVM(self.ComplainStatusId(), self.ArabicName(), self.EnglishName());
    }

    self.toSubmitModel = function () {
        return {
            ComplainStatusId: self.ComplainStatusId(),
...
function ComplainStatusMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            return new ComplainStatusVM(item.ComplainStatusId, item.ArabicName, item.EnglishName);
        });
    };
    ...
    self.ComplainStatuses = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.ComplainStatuses().length === 0);
    });
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.ComplainStatuses(), function (element) {
            return element.ComplainStatusId() === self.SelectedElement().ComplainStatusId();
        });
    };
    ...
    self.GetSelectedElementOrFirstOrNew = function () {
        ...
        if (self.IsItemsEmpty() === false) {
            self.EnterDetailsMode();
            return self.ComplainStatuses()[0];
        }
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.ComplainStatuses(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.ComplainStatuses(), function (temp) {
            return temp.ComplainStatusId() === id;
        });
    }
    ...
    self.Delete = function () {
        ...
        $.ajax({
            ...
            url: self.DeleteUrl,
            data: {
                id: self.SelectedElement().ComplainStatusId(),
                criteria: self.SearchCriteria().toSubmitModel()
            },
            success: function (e) {
                self.HandleSuccess(e);
