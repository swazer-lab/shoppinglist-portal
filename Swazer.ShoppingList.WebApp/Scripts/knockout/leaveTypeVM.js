function LeaveTypeSearchCriteriaVM(options) {
    var self = this;
...
function LeaveTypeVM(id, englishName, arabicName, isActive) {
    var self = this;

    self.LeaveTypeId = ko.observable(id);
    ...
    self.copy = function () {
        return new LeaveTypeVM(self.LeaveTypeId(), self.EnglishName(), self.ArabicName(), self.IsActive());
    }
...
function LeaveTypeMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            return new LeaveTypeVM(item.LeaveTypeId, item.EnglishName, item.ArabicName, item.IsActive);
        });
    };
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.LeaveTypes().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new LeaveTypeVM(0, '', '', false);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.LeaveTypes(), function (element) {
            return element.LeaveTypeId() === self.SelectedElement().LeaveTypeId();
        });
    };
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.LeaveTypes(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.LeaveTypes(), function (temp) {
            return temp.LeaveTypeId() === id;
        });
    }
    ...
    self.Delete = function () {
        ...
        $.ajax({
            ...
            url: self.DeleteUrl,
            data: {
                id: self.SelectedElement().LeaveTypeId(),
                criteria: self.SearchCriteria().toSubmitModel()
            },
            success: function (e) {
                self.HandleSuccess(e);
