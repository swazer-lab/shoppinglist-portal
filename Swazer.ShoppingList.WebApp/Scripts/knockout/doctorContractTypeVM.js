function DoctorContractTypeSearchCriteriaVM(options) {
    var self = this;
...
function DoctorContractTypeVM(id, englishName, arabicName, isActive) {
    var self = this;

    self.DoctorContractTypeId = ko.observable(id);
    ...
    self.copy = function () {
        return new DoctorContractTypeVM(self.DoctorContractTypeId(), self.EnglishName(), self.ArabicName(), self.IsActive());
    }

    self.toSubmitModel = function () {
        return {
            DoctorContractTypeId: self.DoctorContractTypeId(),
...
function DoctorContractTypeMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            return new DoctorContractTypeVM(item.DoctorContractTypeId, item.EnglishName, item.ArabicName, item.IsActive);
        });
    };
    ...
    self.DoctorContractTypes = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.DoctorContractTypes().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new DoctorContractTypeVM(0, '', '', false);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.DoctorContractTypes(), function (element) {
            return element.DoctorContractTypeId() === self.SelectedElement().DoctorContractTypeId();
        });
    };
    ...
    self.GetSelectedElementOrFirstOrNew = function () {
        ...
        if (self.IsItemsEmpty() === false) {
            self.EnterDetailsMode();
            return self.DoctorContractTypes()[0];
        }
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.DoctorContractTypes(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.DoctorContractTypes(), function (temp) {
            return temp.DoctorContractTypeId() === id;
        });
    }
    ...
    self.Delete = function () {
        ...
        $.ajax({
            ...
            url: self.DeleteUrl,
            data: {
                id: self.SelectedElement().DoctorContractTypeId(),
                criteria: self.SearchCriteria().toSubmitModel()
            },
            success: function (e) {
                self.HandleSuccess(e);
