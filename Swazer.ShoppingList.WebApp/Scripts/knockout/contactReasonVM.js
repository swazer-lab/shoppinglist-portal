function ContactReasonSearchCriteriaVM(options) {
    var self = this;
...
function ContactReasonVM(id, arabicName, englishName, arabicProposedSolution, englishProposedSolution, isActive) {
    var self = this;

    self.IsMarked = ko.observable();
    self.ContactReasonId = ko.observable(id);
    ...
    self.ArabicProposedSolution = ko.observable(arabicProposedSolution);
    self.EnglishProposedSolution = ko.observable(englishProposedSolution);
    self.IsActive = ko.observable(isActive).extend({ required: true })

    self.copy = function () {
        return new ContactReasonVM(self.ContactReasonId(), self.ArabicName(), self.EnglishName(), self.ArabicProposedSolution(), self.EnglishProposedSolution(), self.IsActive());
    }

    self.toSubmitModel = function () {
        return {
            ContactReasonId: self.ContactReasonId(),

            ArabicName: self.ArabicName(),
            EnglishName: self.EnglishName(),

            ArabicProposedSolution: self.ArabicProposedSolution(),
            EnglishProposedSolution: self.EnglishProposedSolution(),
...
function ContactReasonMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            var result = new ContactReasonVM(item.ContactReasonId, item.ArabicName, item.EnglishName, item.ArabicProposedSolution, item.EnglishProposedSolution, item.IsActive);

            if (self.IsMarkedContactReason(item.ContactReasonId))
    ...
    self.IsMarkedContactReason = function (id) {
    ...
    self.ContactReasons = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.ContactReasons().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new ContactReasonVM(0, '', '', '', '', false);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.ContactReasons(), function (element) {
            return element.ContactReasonId() === self.SelectedElement().ContactReasonId();
        });
    };
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.ContactReasons(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.ContactReasons(), function (temp) {
            return temp.ContactReasonId() === id;
        });
    }

    self.SelectedItems = ko.pureComputed(function () {
        self.SelectedItemsIds([]);
        return ko.utils.arrayFilter(self.ContactReasons(), function (r) {
            if (r.IsMarked() === true)
                self.SelectedItemsIds().push(r.ContactReasonId());
    ...
    self.ChangeStatusSingle = function () {
        var id = [self.SelectedElement().ContactReasonId()];
