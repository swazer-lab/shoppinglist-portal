function MedicalCenterCategorySearchCriteriaVM(options) {
    var self = this;
...
function MedicalCenterCategoryVM(id, englishName, arabicName, isActive) {
    var self = this;

    self.MedicalCenterCategoryId = ko.observable(id);
    ...
    self.copy = function () {
        return new MedicalCenterCategoryVM(self.MedicalCenterCategoryId(), self.EnglishName(), self.ArabicName(), self.IsActive());
    }

    self.toSubmitModel = function () {
        return {
            MedicalCenterCategoryId: self.MedicalCenterCategoryId(),
...
function MedicalCenterCategoryMainVM(options) {
    var self = this;
    ...
    self.mapItems = function (collection) {
        ...
        for (var i = 0; i < collection.length; i++) {
            var it = collection[i];
            var op = new MedicalCenterCategoryVM(it.MedicalCenterCategoryId, it.EnglishName, it.ArabicName, it.IsActive);

            if (self.IsMarkedMedicalCenterCategory(it.MedicalCenterCategoryId))
                op.IsMarked(true);
            else
                op.IsMarked(false);
    ...
    self.mapItems = function (collection) {
        return ko.utils.arrayMap(collection, function (item) {
            var result = new MedicalCenterCategoryVM(item.MedicalCenterCategoryId, item.EnglishName, item.ArabicName, item.IsActive);

            if (self.IsMarkedMedicalCenterCategory(item.MedicalCenterCategoryId))
    ...
    self.IsMarkedMedicalCenterCategory = function (id) {
    ...
    self.MedicalCenterCategories = ko.observableArray();
    ...
    self.IsItemsEmpty = ko.pureComputed(function () {
        return (self.MedicalCenterCategories().length === 0);
    });
    ...
    self.CreateNewElement = function () {
        return new MedicalCenterCategoryVM(0, '', '', false);
    };
    ...
    self.FindSelectedElement = function () {
        ...
        return ko.utils.arrayFirst(self.MedicalCenterCategories(), function (element) {
            return element.MedicalCenterCategoryId() === self.SelectedElement().MedicalCenterCategoryId();
        });
    };
    ...
    self.GetSelectedElementOrFirstOrNew = function () {
        ...
        if (self.IsItemsEmpty() === false) {
            self.EnterDetailsMode();
            return self.MedicalCenterCategories()[0];
        }
    ...
    self.HandleCreateSuccess = function (e) {
        ...
        self.MedicalCenterCategories(observableItems);
    ...
    self.FindById = function (id) {
        return ko.utils.arrayFirst(self.MedicalCenterCategories(), function (temp) {
            return temp.MedicalCenterCategoryId() === id;
        });
    }

    self.SelectedItems = ko.pureComputed(function () {
        self.SelectedItemsIds([]);
        return ko.utils.arrayFilter(self.MedicalCenterCategories(), function (m) {
            if (m.IsMarked() === true)
                self.SelectedItemsIds().push(m.MedicalCenterCategoryId());
    ...
    self.ChangeStatusSingle = function () {
        var id = [self.SelectedElement().MedicalCenterCategoryId()];
